...
function getBadgeInfo(hours: number) {
  if (hours >= 160)
    return { label: "MISSION IMPOSSIBLE", class: "badge-impossible", icon: faGhost };
  if (hours >= 130)
    return { label: "GOD LEVEL", class: "badge-god", icon: faCrown };
  if (hours >= 100)
    return { label: "STARLIGHT", class: "badge-starlight", icon: faStar };
  if (hours >= 50)
    return { label: "ELITE", class: "badge-elite", icon: faFire };
  if (hours >= 20)
    return { label: "PRO", class: "badge-pro", icon: faBolt };
  if (hours >= 5)
    return { label: "NOVICE", class: "badge-novice", icon: faMedal };
  if (hours >= 1)
    return { label: "NEWBIE", class: "badge-newbie", icon: faSeedling };
  return { label: "NONE", class: "badge-none", icon: faMinus };
}
          ...
          <div className="glass-card border-white/5 bg-white/[0.015] rounded-2xl overflow-hidden">
            ...
            <div className="divide-y divide-white/5">
              {rankedTopMembers.map((member, i) => {
                const rank = i + 1;
                const badgeInfo = getBadgeInfo(member.total_seconds / 3600);
                ...
                return (
                  ...
                  >
                    ...
                    <div className="hidden md:flex md:justify-start">
                      <div
                        className={`badge-base ${badgeInfo.class} shrink-0 !text-[9px] !py-0.5 !px-2`}
                      >
                        {badgeInfo.icon && (
                          <FontAwesomeIcon
                            icon={badgeInfo.icon}
          ...
          <div className="glass-card mt-6 p-5 rounded-2xl border-white/5 bg-white/[0.015]">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
              {[
                ...
                const b = getBadgeInfo(item.hours);
                return (
                  <div
                    key={item.hours}
                  ...
                  >
                    <div
                      className={`badge-base ${b.class} shrink-0 !text-[9px] !py-0.5 !px-2`}
                    >
                      {b.icon && (
